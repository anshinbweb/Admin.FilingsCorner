...
const ServiceMaster = () => {
        ...
        getServiceById(_id)
            .then((res) => {
                ...
                setReqDocs(
                    reqDocsList
                        .filter((doc) =>
                            res.data.requiredDocuments.includes(doc._id)
                        )
                        .map((doc) => ({
                            value: doc._id,
    ...
    const handleClick = (e) => {
        ...
        if (Object.keys(errors).length === 0) {
            const dataToSent = {
                ...
                requiredDocuments: reqDocs.map((doc) => doc.value),
            };
            createService(dataToSent)
                .then((res) => {
                    setmodal_list(!modal_list);
                    setValues(initialState);
                    setIsSubmit(false);
    ...
    document.title = `Service Master | ${adminData.data.CompanyName}`;
